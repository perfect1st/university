...
import { GET_ALL_DEPARTMENTS_IN_FACULTY_BY_ID, GET_FACULITY_BY_ID, UPDATE_FACULITY_DEPARTMENT_BY_ID } from "../../graphql/facultyQuiries";
import notify from "../../components/notify";

export default function AllDepartmentsInFaculityPage() {
    ...
    ] = useLazyQuery(GET_FACULITY_BY_ID, { fetchPolicy: "network-only" });
    ...
    const fetchAndExport = async (type) => {
        try {
            const exportData = getFacultyDepartmentsByFaculty?.map((user) => ({
                ID: user.serial_num,
                "Full Name": user.name,
                Email: user.email,
                Mobile: user.mobile,
    ...
    if (departmentsLoading || facultyLoading) return <LoadingPage />
    return (
        <Box sx={{ p: 3, backgroundColor: "background.paper" }}>
            ...
            <Grid container spacing={3}>
                ...
                >
                    <Header
                        title={t("departments")}
                        subtitle={`${t("departments")} (${isArabic ? faculty?.title_ar : faculty?.title_en})`}
                        i18n={i18n}
