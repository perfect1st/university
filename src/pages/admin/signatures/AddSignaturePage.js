...
import { CREATE_SIGNATURE } from "../../../graphql/signatures";
...
export default function AddSignaturePage() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        await CreateSignature({ variables: { input } });
  ...
  return (
    <Box sx={{ p: 3, backgroundColor: "background.paper" }}>
      <Header
        ...
        subtitle={isArabic ? "إضافة توقيع" : "Add Signature"}
        i18n={i18n}
        haveBtn={false}
        hasAddOrEditBtn={true}
        sub2={isArabic ? "إضافة توقيع" : "Add Signature"}
        hasNavigate={true}
      />
