...
import { GET_ALL_STUDENT_DEGREES } from "../../graphql/studentDegreeQueries";
...
export default function AllStudentDegreesPage() {
    ...
    const [StudentDegreesAll, { data: { studentDegreesAll } = {}, loading }] =
        ...
        <Box sx={{ p: 3, backgroundColor: "background.paper" }}>
            <Grid container spacing={3}>
                <Grid item xs={12}>
                        ...
                        <Box key={sIndex} sx={{ mt: 5 }}>
                            ...
                            <TableContainer component={Paper} sx={{ boxShadow: "0px 4px 20px rgba(0, 0, 0, 0.05)", borderRadius: 2 }}>
                                <Table stickyHeader aria-label="collapsible table">
                                    ...
                                    <TableBody>
                                        {studentEntry.subjects?.map((subject, subIndex) => (
                                            <SubjectGroup
                                                key={`${sIndex}-${subIndex}`}
                                                subject={subject}
                                                isArabic={isArabic}
                                                t={t}
                                                studentSerial={studentEntry.student?.serial}
                                            />
                                        ))}
                                    </TableBody>
                                </Table>
                    ...
                    {(!studentDegreesAll || studentDegreesAll.length === 0) && (
